...
//ReadFrequencyTable takes a frequency table file as input and return a frequen table.
...
func main() {
	...
	if augs[0] == "read" {
		prefixLen,err1 := strconv.Atoi(augs[1])
		fileName := augs[2]
		if err1 != nil {
			panic("something wrong in converting string to integer")
		}
	...
	if augs[0] == "generate" {
		inFile := augs[1]							 //the second augement is the input file
		...
		WriteStringsToFile(text, outFile)             // Write text to standard output.
	}
}
